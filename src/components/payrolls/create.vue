...
<script>
    ...
    export default {
        ...
        computed: {
            grey(){
                if(this.payroll.user_id=='' || this.payroll.user_id==null || this.payroll.user_id==undefined){
                    return true 
                }else{
                    return false
                }
        ...
        methods: {
            ...
            close () {
                ...
                this.payroll.date=''
                this.payroll.user_id=''
                this.payroll.imss=0
                this.payroll.infonavit=0
                this.payroll.amount=0
                this.payroll.extra_time=0
                this.payroll.production_award=0
                this.payroll.punctuality_award=0
                this.payroll.performance_award=0
                this.payroll.absence=0
                this.payroll.notes=''

                if(this.company!=undefined){
                    this.payroll.company_id=Number(this.company)
                }
                this.gris = false
                this.$nextTick(() => {
                    this.$emit("closeCreateDialogPayroll", false);
                })
            },
            save(){
                this.payroll.created_by_user_id = this.currentUser.id
                this.payroll.last_updated_by_user_id = this.currentUser.id
                axios.post(process.env.VUE_APP_BACKEND_ROUTE + "api/v2/payrolls",Object.assign(this.payroll)).then(response=>{
                    this.close()
                }).catch(error => {
                    this.snackbar = {
