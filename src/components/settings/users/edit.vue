                ...
                <v-stepper-items>
                    <v-stepper-content step="1" class="pt-0">
                        <v-row class="mx-0">
                            ...
                            <v-col cols="12" sm="6" md="4">
                                <v-select v-model="editedItem.rol" :items="roles" item-value="id" item-text="name" label="Rol"></v-select>
                            </v-col>
                        </v-row>
                        <v-row class="mx-0">
...
export default {
    ...
    methods:{
        ...
        save () {
            axios.patch(process.env.VUE_APP_BACKEND_ROUTE + "api/v2/users", this.editedItem).then(response=>{
                this.close()
            }).catch(error => {
                this.snackbar = {
