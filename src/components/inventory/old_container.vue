...
<keep-alive>
    <v-container style="max-width:100vw;">
        ...
        <v-dialog v-model="createDialog" max-width="700px">
          ...
          <createFelItem v-if="liga == 'https://backend.ferreteriaenlinea.com/'" @closeDialogItem="closeDialogItem"/>
          <createItem v-else @closeDialogItem="closeDialogItem"/>
        </v-dialog>
        <!-- editar -->
        <v-dialog v-model="editDialog" max-width="600px">
            <editFelItem v-if="liga == 'https://backend.ferreteriaenlinea.com/'" @closeDialogItem="closeDialogItem" v-bind:editedItem="editedItem"/>
...
export default {
    ...
    methods:{
        sumField(key) {//suma
            var suma=this.items.reduce((a, b) => a + ((b[key]*1) || 0), 0)
            if(key=='inventory'){
                return suma + ' kg'
            }else{
